#!/usr/bin/env python3
"""Liquidity-aware Kalshi hunter — only trades on real depth, uses proven order path."""
import sys
import time
...
def log(msg: str) -> None:
    ...
    try:
        log_path = Path("logs/liquidity_hunter.log")
...
def buy(ticker: str, price_cents: float) -> bool:
    try:
        status, resp = kalshi_post_order({
            ...
            "client_order_id": f"hunt-buy-{int(time.time()*1000)}",
            "type": "limit",
        })
...
def sell(ticker: str, price_cents: float) -> bool:
    try:
        status, resp = kalshi_post_order({
            ...
            "client_order_id": f"hunt-sell-{int(time.time()*1000)}",
            "type": "limit",
        })
        fill = float(resp.get("fill_count", 0))
        if fill > 0:
...
def main() -> None:
    log("=== LIQUIDITY HUNTER STARTING ===")
    balance = get_balance()
    log(f"Initial balance: ${balance:.2f}")

    while True:
        try:
            balance = get_balance()
            log(f"cash=${balance:.4f} positions={len(position)}")
            ...
            if markets:
                log(f"LIQUIDITY FOUND: {len(markets)} markets")
                for m in markets[:3]:
