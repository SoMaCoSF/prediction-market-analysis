...
def encode_gyst(
    *,
    ...
    content_seed: optional extra entropy mixed into the deterministic low-42.
    Same seed -> same UUID (reproducible); distinct seeds -> distinct UUIDs.
    Use it for entities that must be unique despite identical structured fields
    (e.g. two orders on the same ticker/price in the same second). The seed is
    NOT recoverable from the UUID (SHA-256 is one-way) — reconciliation uses the
    decoded low-42 value itself as the key.
    """
    ...
    parts = [type_code, namespace, ts, fractal, sig_q, prov]
    if content_seed is not None:
        parts.append(content_seed)
    r42 = _content42(*parts)
